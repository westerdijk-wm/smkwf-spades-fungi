...
def get_platform(wildcards):
    """Get platform of given sample-unit."""
    row = units.loc[(wildcards.sample, wildcards.unit), ["platform"]].dropna()
    if len(row) == 1:
        return row.platform
    return "ILLUMINA"
...
def get_sample_bams_recal(wildcards):
...
def get_sample_bai_dedup(wildcards):
    """Get all aligned reads of given sample."""
    return expand(
